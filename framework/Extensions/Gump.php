...
class Gump
{
	...
	public function __construct($lang = 'en', $langDirectory = "")
	{
		if ($lang) {
			$this->langDirectory = $langDirectory;
			if (empty($langDirectory)) {
				$lang_file = dirname(__FILE__) . '/../language/gump/' . $lang . '.php';
			} else {
				$lang_file = $langDirectory . "/" . $lang . '.php';
			}

			if (file_exists($lang_file)) {
				$this->lang = $lang;
			} else {
				throw new \Exception('Language with key "' . $lang_file . '" does not exist');
			}
		}
	}
	...
	{
		if (empty($this->langDirectory)) {
			$lang_file = dirname(__FILE__) . '/../language/gump/' . $this->lang . '.php';
		} else {
			$lang_file = $this->langDirectory . "/" . $this->lang . '.php';
		}
	...
	{
		if (isset($input[$field]) && ($input[$field] === "0" || $input[$field] === false || $input[$field] === 0 || $input[$field] === 0.0 || $input[$field] === '0' || !empty($input[$field]))) {
			return;
		}
	 ...
	 * @return mixed
	 */
	protected function validate_valid_tckno($field, $input, $param = null)
	{
		if (!isset($input[$field]) || empty($input[$field])) {
			return;
		}
	...
	/**
	 * Determine if the provided value is a valid mobile phone number.
	 *
	 * Usage: '<index>' => 'phone_number'
	 *
	 * @param string $field
	 ...
	 *  1-555-555-5555: valid
	 *  1-(555)-555-5555: valid
	 *  0 (555) 555 55 55: valid
	 */
	protected function validate_mobilephone_number($field, $input, $param = null)
	{
		if (!isset($input[$field]) || empty($input[$field])) {
			return;
		}

		$regex = '/^(\d[\s\s|-]?)?[\(\[\s\s|-]{0,2}?\d{3}[\)\]\s-]{0,2}?\d{3}[\s-]?\d{2}[\s-]?\d{2}$/i';
		if (!preg_match($regex, $input[$field])) {
			return [
				'field' => $field,
				'value' => $input[$field],
