...
class UserAgent
{
    ...
    public function parse()
    {
        ...
        if (!$this->u_agent) return $empty;

        if (preg_match('/\((.*?)\)/im', $this->u_agent, $parent_matches)) {
        ...
        preg_match_all('%(?P<browser>Camino|Kindle(\ Fire)?|Firefox|Iceweasel|IceCat|Safari|MSIE|Trident|AppleWebKit|
				...
				(?:\)?;?)
				(?:(?:[:/ ])(?P<version>[0-9A-Z.]+)|/(?:[A-Z]*))%ix', $this->u_agent, $result, PREG_PATTERN_ORDER);

        // If nothing matched, return null (to avoid undefined index errors)
        if (!isset($result['browser'][0]) || !isset($result['version'][0])) {
            if (preg_match('%^(?!Mozilla)(?P<browser>[A-Z0-9\-]+)(/(?P<version>[0-9A-Z.]+))?%ix', $this->u_agent, $result)) {
        ...
        if (preg_match('/rv:(?P<version>[0-9A-Z.]+)/si', $this->u_agent, $rv_result)) {
