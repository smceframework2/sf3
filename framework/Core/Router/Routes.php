...
class Routes implements RouteInterface
{
    ...
    public function via($httpMethods)
    {
	    ...
	    if (is_string($httpMethods)&&$httpMethods=='*') $httpMethods='GET,POST,OPTION,OPTIONS,PUT,DELETE,PATCH,FETCH';
        if (is_array($httpMethods)&&$httpMethods[0]=='*') $httpMethods=explode(',','GET,POST,OPTION,OPTIONS,PUT,DELETE,PATCH,FETCH');
    ...
    public function reConfigure($pattern, $paths = null)
    {
        ...
        if (is_string($paths) === true) {
            ...
            if (is_null($controllerName) === false) {
                ...
                $realClassName = $this->uncamelize($realClassName);
        ...
        if (self::startsWith($pattern, '#') === false) {
            if (strpos($pattern, '{') !== false) {
