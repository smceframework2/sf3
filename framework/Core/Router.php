...
class Router
{
	...
	public function __construct($defaultRoutes = null)
	{
		...
		if ($defaultRoutes === true) {
			 ...
			 */
			$routes[] = new Routes('#^/([a-zA-Z0-9\\_\\-]+)[/]{0,1}$#', ['controller' => 1]);
			$routes[] = new Routes(
	 ...
	 */
	private static function EremoveExtraSlashes($str)
	{
	...
	public function handle($uri = null)
	{
		...
		if ($this->_removeExtraSlashes === true) {
			$uri = self::EremoveExtraSlashes($uri);
		}
		if (strpos($uri, '?') !== false) {
		...
		$routes = (is_array($this->_routes) === true ? $this->_routes : []);
		...
		foreach (array_reverse($this->_routes) as $route) {
			...
			if ($routeFound === true) {
				...
				if (is_null($beforeMatch) === false) {
					...
					//Call the function
					$routeFound = call_user_func_array($beforeMatch, [$uri, $route, $this]);
				}
			}
			...
			if ($routeFound === true) {
				...
				if (is_array($middlewares)) {
					foreach ($middlewares as $key => $value) {
						...
						if (method_exists($class_, "handle") && !$class_->handle($this->_request)) {
							$routeFound = false;
							if (method_exists($class_, "terminate")) {
								return $class_->terminate($this->_response);
							}
		...
		if ($routeFound === true) {
			...
			if (isset($parts["closure"])) {
				$this->_closure = $parts["closure"];
			}
			...
			if (isset($parts['params']) === true) {
				$paramStr = (string) substr($parts['params'], 1, strlen($parts['params']));
	...
	/**
	 * Adds a route to the router that only match if the any HTTP method
	 *
	 * @param string $pattern
	 * @param string|array|null $paths
	 * @return \EF2\Core\Router\Route
...
//		pre($args);
		if (strpos($args[0], '@')) {
			list($controller, $action) = explode('@', $args[0]);
			if (isset($args[1]))
				list(, $params) = $args;
		} elseif (count($args) == 3) {
		...
		if (count($params) > 0) {
			...
			if (isset($params)) {
				...
				DI::bind('router', function () use ($router) {
					return $router;
				});
			}
		}
		//die('$controller = '.$controller.'Controller::class');
		eval('$controller = ' . $controller . 'Controller::class;');
		$action = 'action' . Str::ucfirst($action);

		DISingleton::make($controller, $action);
	}

	public function route($file)
	{
		if (file_exists($file)) {
			if (isset($this->_routes[0]))
				unset($this->_routes[0]);

			if (isset($this->_routes[1]))
				unset($this->_routes[1]);

			require_once $file;
		} else {
			throw new Exception("not route file " . $file);
		}
	}
	...
	public function setRoutes()
	{
		...
		foreach ($routes as $key => $value) {
			if ($key == "notfound") {
				$this->notFound($value["params"]);
			} else {
				$r = $this->add($value["pattern"], $value["params"])->via([$value["via"]]);
				$r->setMiddlewares($value["middleware"]);
			}
		}
	}
}